...
已定的三条前提（§18）：**待办上收为内核服务**、**首版做完整形态**（含第三方模块仓库与隔离）、**默认目录保持为空、凭据不进仓库**。
...
用户的工作情况走「工作情况」设置，不写进默认代码。启动默认是空目录。
...
| `micro` 的来源 / 领域清单 | 通用公开源，不是真实站点清单 |
| `main/micro-sourcing-fetch.ts` 的 User-Agent | 只用产品名和版本，不带真实站点 |
...
3. **默认目录保持为空，凭据不进仓库**。§15 的「硬编码常量配置化」是模块化本身的要求；许可证、i18n、治理文件见本仓库（§15.1）。
